class CylinderBuilding extends Building{
  constructor(cx, cy, cz, x, y, z, building_color, max_displacement, horizontal_lines=5){
    super(cx, cy, cz, building_color);
    this.x_length = x;
    this.y_length = y;
    this.z_length = z;
    this.radius = Math.max(this.x_length/2, this.y_length/2);
    this.segments = Math.min(this.x_length/5, this.y_length/5);
  ...
  draw_windows() {
    ...
    const topPoints = [];
    const bottomPoints = [];
    const midPoints = Array.from({ length: this.horizontal_lines }, () => []);
    ...
    for (let i = 0; i < this.segments; i++) {
        ...
        topPoints.push([x + disp(), this.z_length / 2 + disp(), y + disp()]);
        bottomPoints.push([x + disp(), -this.z_length / 2 + disp(), y + disp()]);
